...
class Profiler
{
    ...
    //初始化$defaultOptions，用于传参到tideways_enable方法
    /**
    ...
    //指示Tideways Profiler在分析期间自动检测事务名称。
    /**
    ...
    public static function start($options = array(), $sampleRate = null)
    {
        ...
        //add by gaochy8
        $_SERVER['TIDEWAYS_APIKEY']='yyyTideways';

        //从$_SERVER或者php.ini中读取api_key、sample_rate等值
        $defaults = array(
    ...
    private static function enableProfiler($mode)
    {
        ...
        //根据mode，设置对应的$flags，用于在tideways_enable中传入参数
    ...
    private static function decideProfiling($treshold, array $options = array())
    {
        ...
        //通过给定的mode，开始采样，内部调用tideways_enable
